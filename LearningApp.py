...
def **init**(self, camera_id=0, sensitivity=25, photo_interval=1.5,
motion_timeout=3.0, resolution=(160, 120), fps=3):
“””
Initialize self-learning motion detector
“””
...
def load_motion_patterns(self):
    """Load motion pattern recognition data"""
    try:
...
def analyze_lighting_conditions(self, frame):
    """Analyze and learn from lighting conditions"""
    # Calculate average brightness
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    avg_brightness = np.mean(gray)
    brightness_std = np.std(gray)
    
    # Store lighting condition
    lighting_info = {
...
def calculate_motion_confidence(self, motion_boxes, motion_percent, brightness):
    """Calculate confidence level for detected motion"""
    confidence = 0.5  # Base confidence
    
    # Factor 1: Motion area size
    if motion_boxes:
        total_area = sum(w * h for _, _, w, h in motion_boxes)
        area_ratio = total_area / self.frame_area
        
        # Optimal motion area is 5-30% of frame
        if 0.05 <= area_ratio <= 0.3:
            confidence += 0.2
        elif area_ratio < 0.05:
            confidence -= 0.1  # Too small, might be noise
        elif area_ratio > 0.5:
            confidence -= 0.2  # Too large, might be lighting change
    
    # Factor 2: Motion consistency
    if len(self.motion_history) >= 3:
        ...
        if motion_variance < 0.1:  # Consistent motion
            confidence += 0.15
    
    # Factor 3: Lighting stability
    if self.ambient_light_baseline and brightness:
        light_diff = abs(brightness - self.ambient_light_baseline)
        if light_diff < 20:  # Stable lighting
            confidence += 0.1
        elif light_diff > 50:  # Unstable lighting
            confidence -= 0.15
    ...
    # Factor 5: Motion box characteristics
    if motion_boxes:
        # Prefer rectangular, medium-sized objects
...
def adapt_sensitivity(self):
    ...
    if avg_confidence < 0.4:  # Low confidence, increase sensitivity
        self.current_sensitivity = max(10, self.current_sensitivity * 0.9)
        print(f"📈 Sensitivity increased to {self.current_sensitivity:.1f}")
    elif avg_confidence > 0.8:  # High confidence, can decrease sensitivity
    ...
    # Update motion threshold based on environment
    if self.ambient_light_baseline:
        ...
        if recent_brightness:
            ...
            # Adjust threshold for lighting conditions
            if light_ratio < 0.5:  # Low light
                self.motion_threshold = 0.2
            elif light_ratio > 1.5:  # Bright light
                self.motion_threshold = 0.4
            else:  # Normal light
                self.motion_threshold = 0.3

def learn_from_detection(self, motion_boxes, motion_percent, confidence):
    """Learn from each detection event"""
    ...
    # Update motion confidence history
...
def initialize_camera(self):
    """Initialize camera with learning-optimized settings"""
    ...
    # Apply learned exposure if available
...
def detect_motion_with_learning(self, frame):
    """Enhanced motion detection with learning"""
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
    ...
    # Calculate difference with adaptive threshold
    ...
    # Check for motion using adaptive threshold
    ...
    # Only consider high-confidence detections as real motion
...
def save_photo_with_learning(self, frame, confidence):
    """Save photo with learning metadata"""
    current_time = time.time()
...
def run(self):
    """Main detection loop with learning"""
    ...
    try:
        while self.running:
            ...
            # Periodic learning save and status
...
def cleanup(self):
    """Clean up and save final learning state"""
    self.running = False
    if self.cap:
        self.cap.release()
    
    # Save final learning progress
    self.save_learning_progress()
